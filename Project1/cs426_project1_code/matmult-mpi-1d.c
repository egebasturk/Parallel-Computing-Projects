    ...
    for (int row = 0; row < dim1; ++row) {
        for (int column = 0; column < dim1; ++column) {
            for (int i = 0; i < dim2; ++i) {
                sum_tmp += mat1[row * dim2 + i] * mat2[column + i * dim2];
            }
            //printf("col:%d/%d row:%d/%d\n", column, dim1, row, dim1);
    ...
    */
    ...
    if (rank_row % N == 0)
    {
        ...
        */
        ...
        MPI_Scatter(array1
                , N, MPI_INT, recv_buffer
                , N, MPI_INT, 0, master_comm);
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Bcast(recv_buffer + offset, N, MPI_INT, 0, comm_row);
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Gather(partial_result, N * grid_side, MPI_INT
